...
class LinearDecomMPO(nn.Module):
    ...
    def forward(self, x):
        ...
        t = self.tensor_set[0]
        for i in range(1, self.mpo.num_dim):
            t = torch.tensordot(t, self.tensor_set[i], ([len(t.shape)-1],[0]))
        # Squeeze the first and the last 1 dimension
        t = t.squeeze(0)
        t = t.squeeze(-1)
        # Transpose and reshape to output
        new_index = self.mpo.reverse_index_permute
        t = t.permute(tuple(new_index))
        weight = t.reshape(self.mpo.input_shape, self.mpo.output_shape)

        res = F.linear(res, weight,self.bias)
        ##################### use rebuild
        ori_shape=x.shape
    ...
    def from_pretrained(self, input_shape,kernel_pretrain,tensor_set,bias=None,use_bias=True):
        ...
        self.tensor_set = torch.nn.ParameterList([nn.Parameter(i) for i in tensor_set])
...
class MPOattention(LinearDecomMPO):
    ...
    def from_pretrained(self,input_shape,tensor_set,w_q=None, w_k_trans=None, bias_q=None, bias_k=None, use_bias=False):
        self.tensor_set = torch.nn.ParameterList([nn.Parameter(torch.from_numpy(i).cuda(), requires_grad=True) for i in tensor_set]) 
        if self.tensor_learn:
