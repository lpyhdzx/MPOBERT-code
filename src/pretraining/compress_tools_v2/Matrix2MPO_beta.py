...
class MPO:
    def __init__(self, mpo_input_shape, mpo_output_shape, truncate_num, fix_rank=None):
        ...
        self.index_permute = tuple(np.transpose(
            np.arange(len(self.mpo_input_shape) + len(self.mpo_output_shape)).reshape((2, -1))).flatten())
        tmp1 = torch.tensor(range(len(self.mpo_output_shape))) * 2
        tmp2 = tmp1 + 1
        self.reverse_index_permute = tuple(torch.cat((tmp1, tmp2), 0))
        self.input_shape = torch.prod(torch.tensor(self.mpo_input_shape))
        self.output_shape = torch.prod(torch.tensor(self.mpo_output_shape))
    ...
    def get_tensor_set(self, inp_matrix):
        ...
        for i in range(self.num_dim - 1):
            # Do the SVD operator
            res = res.view([self.mpo_ranks[i] * self.mpo_input_shape[i] * self.mpo_output_shape[i], -1])
            u, lamda, v = torch.linalg.svd(res, full_matrices=False)
            # The first tensor should be T1(r_i+1, m_i, n_i, r_i)
            u = u.view([self.mpo_ranks[i], self.mpo_input_shape[i], self.mpo_output_shape[i], self.mpo_ranks[i+1]])
            tensor_set.append(u)
            res = (lamda.view(-1, 1) * v).contiguous()
        res = res.view([self.mpo_ranks[self.num_dim-1], self.mpo_input_shape[self.num_dim-1],
    ...
    def left_canonical(self,tensor_set):
        ...
        left_canonical_tensor[1] = lamda.view(-1, 1) * v
        for i in range(1, self.num_dim - 1):
            mat = torch.tensordot(left_canonical_tensor[i], tensor_set[i], [[1], [0]])
            mat = mat.view(-1, mat.size(-1))
            u, lamda, v = torch.linalg.svd(mat, full_matrices=False)
            left_canonical_tensor[i + 1] = lamda.view(-1, 1) * v
    ...
    def right_canonical(self, tensor_set):
        ...
        u, lamda, v = torch.linalg.svd(mat, full_matrices=False)
        right_canonical_tensor[self.num_dim - 1] = u * lamda
        for i in range(self.num_dim - 2, 0, -1):
            mat = torch.tensordot(tensor_set[i], right_canonical_tensor[i + 1], [[-1], [0]])
            mat = mat.view(mat.size(0), -1)
            u, lamda, v = torch.linalg.svd(mat, full_matrices=False)
            right_canonical_tensor[i] = u * lamda
    ...
    def gauge_aux_p_q(self, left_canonical_tensor, right_canonical_tensor):
        ...
        lamda_set_value = [0] * (self.num_dim + 1)
    ...
    def mpo_canonical(self, tensor_set, p, q):
        ...
        for i in range(1, self.num_dim-1):
            tensor_set[i] = torch.tensordot(q[i], tensor_set[i], [[1], [0]])
            tensor_set[i] = torch.tensordot(tensor_set[i], p[i + 1], [[-1], [0]])
        return tensor_set
    ...
    def matrix2mpo(self, inp_matrix, cutoff=True):
        ...
        """
        tensor_set = self.get_tensor_set(inp_matrix.cuda())
    ...
    def mpo2matrix(self, tensor_set):
        ...
        """
        # t = ListTensorDot.apply(*tensor_set)
        ...
        new_index = self.reverse_index_permute
        t = t.permute(tuple(new_index))
        t = t.reshape(self.input_shape, self.output_shape)
