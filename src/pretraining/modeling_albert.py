...
ACT2FN = {"gelu": F.gelu, "relu": F.relu, "swish": swish, "tanh": F.tanh, "gelu_new": gelu_new}
...
def get_apex_layer_norm():
    try:
        ...
        logger.info("Layer norm of type apex is not available, apex not installed.")
        return None
...
class DeepNorm(torch.nn.Module):
    '''
    Implement DeepNorm from "DeepNet: Scaling Transformers to 1,000 Layers"
    Note: Use Encoder architectures, **Only for BERT**!!!
    ref:https://arxiv.org/pdf/2203.00555.pdf
    '''
    def __init__(self, dim, eps, num_encoder_layer, num_decoder_layer):
        super(DeepNorm, self).__init__()

        deep_norm_coefficients, _ = get_deepnorm_coefficients(encoder_layers=num_encoder_layer, decoder_layers=num_decoder_layer)
        self.alpha = deep_norm_coefficients.alpha
        layer_norm = get_apex_layer_norm()
        self.layer_norm = layer_norm(dim, eps)

    def forward(self, x):
        return self.layer_norm(x)

LAYER_NORM_TYPES = {"pytorch": nn.LayerNorm, "apex": get_apex_layer_norm(), "rms_norm": RMSNorm, "deepnorm": DeepNorm}
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.layernorm_embedding:
            ...
            if config.layer_norm_type == 'deepnorm':
                self.LayerNorm = BertLayerNorm(config.embedding_size, eps=1e-12, num_encoder_layer=config.num_hidden_layers, num_decoder_layer=0)
            else:
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        if "FFN_1" in config.mpo_layers:
            ...
            # self.mpo_input_shape, self.mpo_output_shape = [4,4,8,8,4],[4,4,4,4,4] # 1024 * 4096 需要反过来配置
            ...
            self.dense_mpo = LinearDecomMPO(self.mpo_input_shape, self.mpo_output_shape, config.linear_trunc,tensor_learn=config.tensor_learn,CT_learn=config.CT_learn)
            self.from_pretrained_mpo()
        else:
            self.dense = nn.Linear(config.hidden_size, config.intermediate_size)
...
class BertOutput(nn.Module):
    def __init__(self, config):
        ...
        if "FFN_2" in config.mpo_layers:
            ...
            # self.mpo_input_shape, self.mpo_output_shape = [4, 4, 4, 4, 4],[4, 4, 8, 8, 4] # 4096 * 1024
            # self.mpo_input_shape, self.mpo_output_shape = [2,4,4,4,4,2],[4,4,4,4,4,3]
            # self.mpo_input_shape, self.mpo_output_shape = [2,2,2,2,3,2,2,2,2],[2,2,2,4,4,3,2,2,2]
            self.mpo_input_shape, self.mpo_output_shape = config.default_shape[str(config.hidden_size)][str(config.n_local)], config.default_shape[str(config.hidden_size * 4)][str(config.n_local)]
            self.dense_mpo = LinearDecomMPO(self.mpo_input_shape, self.mpo_output_shape, config.linear_trunc,tensor_learn=config.tensor_learn,CT_learn=config.CT_learn)
            self.from_pretrained_mpo()
        else:
            self.dense = nn.Linear(config.intermediate_size, config.hidden_size)
...
class BertLayer(nn.Module):
    def __init__(self, config):
        ...
        BertLayerNorm = get_layer_norm_type(config)
        if config.layer_norm_type == 'deepnorm':
            self.PreAttentionLayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12, num_encoder_layer=config.num_hidden_layers, num_decoder_layer=0)
            self.PostAttentionLayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12, num_encoder_layer=config.num_hidden_layers, num_decoder_layer=0)
        else:
    ...
    def forward(self, hidden_states, attention_mask, action=1, keep_prob=1.0):
        ...
        if action == 0:
            ...
            if self.config.layer_norm_type == 'deepnorm':
            ...
            if self.config.layer_norm_type == 'deepnorm':
                intermediate_input = hidden_states * self.PreAttentionLayerNorm.alpha + attention_output
                intermediate_input = self.PreAttentionLayerNorm(intermediate_input)
            else:
        ...
        if action == 0:
            ...
            if self.config.layer_norm_type == 'deepnorm':
                layer_output = layer_output + intermediate_input * self.PostAttentionLayerNorm.alpha
                layer_output = self.PostAttentionLayerNorm(layer_output)
            else:
...
class BertEncoder(nn.Module):
    def __init__(self, config, args):
        ...
        if config.layer_norm_type == 'deepnorm':
            self.FinalLayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12, num_encoder_layer=config.num_hidden_layers, num_decoder_layer=0)
        else:
...
class BertPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        ...
        BertLayerNorm = get_layer_norm_type(config)
        if config.layer_norm_type == 'deepnorm':
            self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12, num_encoder_layer=config.num_hidden_layers, num_decoder_layer=0)
        else:
...
class AlbertPreTrainingHeads(nn.Module):
    def __init__(self, config, bert_model_embedding_weights):
        ...
        self.predictions = AlbertMLMHead(config, bert_model_embedding_weights)
        self.seq_relationship = AlbertSOPHead(config)
...
class BertForPreTraining(BertPreTrainedModel):
    ...
    def __init__(self, config, args):
        ...
        self.cls = AlbertPreTrainingHeads(config, self.bert.embeddings.word_embeddings.weight)
        # self.apply(self.init_weights)
        self.init_weights()
    def set_names_tobe_shared(self, names_tobe_shared):
        self.names_tobe_shared = names_tobe_shared
    def replace(self, root_name, model=None):
        if not model:
        ...
        for attr_str in dir(model):
            ...
            if type(target_attr) == LinearDecomMPO:
                ...
                if name in self.names_tobe_shared:
                    ...
                    logger.info("\t {} is shared with {}".format(name, self.names_tobe_shared[name]))
                if not shared:
...
class BertLMHeadModel(BertPreTrainedModel):
    ...
    def __init__(self, config, args):
        ...
        self.init_weights() # 调用 modeling_utils.py 初始化函数，包括所有子结构的初始化和tie_weights()
        if config.layer_norm_type == 'deepnorm':
            self._deepnorm_weight_init() # 额外增加Transformer layer的deepnorm初始化，不包括embedding初始化，同xformers
    def _deepnorm_weight_init(self):
        def is_ffn(n): # FFN_1 and FFN_2
            return ("intermediate" in n or "output.dense" in n) and "weight" in n

        def is_attention_output(n):
            return "attention.output" in n and "weight" in n

        def is_value(n):
            return "attention.self.value" in n and "weight" in n

        encoder_coefficients, _ = get_deepnorm_coefficients(
            encoder_layers=len(self.bert.encoder.layer), decoder_layers=0  # type: ignore
        )
        ...
        for n, p in self.bert.encoder.named_parameters():
            if is_ffn(n) or is_attention_output(n) or is_value(n):
                logger.info("Check using DeepNorm for layer -----> {}".format(n))
        ...
        for attr_str in dir(model):
            ...
            if type(target_attr) == LinearDecomMPO:
                ...
                if name in self.names_tobe_shared:
                    shared = True
                    init_attr = eval("self."+route_name_to_module(self.names_tobe_shared[name]))
                    target_attr.tensor_set[2] = init_attr.tensor_set[2]
                    logger.info("\t {} is shared with {}".format(name, self.names_tobe_shared[name]))
                if not shared:
                    logger.info("\t {} is not shared".format(name))
