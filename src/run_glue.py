...
from pretraining.modeling_albert import BertForSequenceClassification as AlbertForSequenceClassification
...
from pretraining.sharer import Sharer, chain_module_names
...
MODELS = {
    "bert-glue": (BertForSequenceClassification, PretrainedBertConfig, AutoTokenizer),
    "albert-fromscratch": (AlbertForSequenceClassification, PretrainedAlbertConfig, AutoTokenizer),
}
...
def main():
    ...
    if data_args.task_name is not None:
        ...
        datasets = load_dataset("/mnt/nlp_data/GLUE/glue.py", data_args.task_name)
    ...
    if layer_idices:
        ...
        print(f"names to be shared: {names_tobe_shared}")
        sharer = Sharer(names_tobe_shared, model)
        sharer.replace(model, root_name="")
        del sharer.model
    else:
        print(f"No parameter sharing")
    module = torch.load(os.path.join(model_args.model_name_or_path,'pytorch_model.bin'))
    model.load_state_dict(module, strict=False)
    
    ###
    # if config.mpo_layers and config.mpo_layers.lower() != "nompo":
    #     print("========== MPO Operation ==========")
    #     if 'FFN_1' in config.mpo_layers:
    #         if 'FFN_1' not in config.load_layer:
    #             for i in range(config.num_hidden_layers):
    ...
    #         config.share_layer = list(map(int, config.share_layer.split(",")))
    #         if config.share_layer:
    #             layer_idices = {config.share_layer[0]:list(range(*config.share_layer))}
    #         else:
    #             layer_idices = {0:list(range(1,config.num_hidden_layers))}
    #     else:
    #         layer_idices = {}
    ...
    #     else:
    #         print(f"No parameter sharing")
    # model.load_state_dict(torch.load(os.path.join(model_args.model_name_or_path,'pytorch_model.bin')), strict=False)
    ###
